				...
				if(target == null){
					p.sendMessage(Utils.getTargetDisconnecte());
					 ...
					 StringBuilder str = new StringBuilder();
					  ...
					  message1.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder( Utils.getMsg_Hover_Message().replace("%player%", target.getName())).create() ) );
					  
					  message2.setClickEvent(new ClickEvent(ClickEvent.Action.SUGGEST_COMMAND, "/msg " + p.getName() + " ") );
					  message2.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(Utils.getMsg_Hover_Message().replace("%player%", p.getName())).create()));
					...
					ModoMessage.getInstance().getReply().put(p, target);
					  for(Player pla : Bukkit.getOnlinePlayers()){
						  if(ModoMessage.getInstance().getSp().contains(pla)){
